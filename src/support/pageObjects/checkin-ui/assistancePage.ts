...
export default class AssistancePage extends CheckinBasePage {
  constructor(page: Page) {
    super(page, 'Is there anything you need support with or want to let us know about?')
  }
  ...
  private textarea(option: AssistanceOptionWithComment): Locator {
    const id = option.toLowerCase().replace(/_(\w)/g, (_: string, p1: string) => p1.toUpperCase()) + 'Support'
    return this.getByID(id)
  }

  /** Selects NO_HELP — mutually exclusive with all other options. */
  ...
  async selectOptionsAndContinue(selections: AssistanceSelection[]): Promise<void> {
