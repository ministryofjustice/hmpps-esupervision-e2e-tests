...
export const luxonString = (date: DateTime): string => date.toFormat('dd/MM/yyyy')

export const dueDateString = (date: DateTime) : string => {
    return date.toFormat("yyyy-M-d")
}

export const getNextWorkingDay = (daysAhead = 1): string => {
  ...
  return luxonString(date)
}

export const uiDateToIso = (uiDate: string): string =>
  DateTime.fromFormat(uiDate, 'dd/MM/yyyy').toISODate()!
