  ...
  await journey.startSetup(crn);
   ...
   const summary = await journey.completeSetupToSummary({
     ...
     await test.step("Change frequency: Every week -> Every 4 weeks", async () => {
        ...
        await expect(summary.summaryValueLocator("frequency")).toContainText("Every 4 weeks")
    })

    await test.step("Change contact preference: Text message -> Email", async () => {
        await journey.changeContactPreferenceFromSummary(summary, Preference.EMAIL)
        await expect(summary.summaryValueLocator("contactPreference")).toContainText("Email")
    })
